...
messages = [
    {"role": "system", "content": "You are a helpful web blogger."},
    {"role": "system", "content": "You are knowledgeable about food and recipes."},
    {"role": "system", "content": "You always produce correctly formatted markdown in your responses."},
    {"role": "system", "content": "You exclusively respond with markdown."},
]

examples = [
    {"role": "user", "content": "Create a recipe for Apple Pie."},
    {"role": "assistant", "content": """# Apple Pie
...
def getRecipeMessageForRecipe(recipe):
    return {
        "role": "user",
        "content": "Create a recipe for " + recipe + "."
    }

def getMarkdownPageForRecipe(recipe):
    print("Generating markdown for " + recipe + "...")
    finalMessages = messages + examples + [getRecipeMessageForRecipe(recipe)]
    chat_completion = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=finalMessages)
    return chat_completion.choices[0].message.content

def kebabLowerCase(string):
    return string.lower().replace(" ", "-")

def writeMarkdownPageToFile(recipe, markdownPage):
    fileName = kebabLowerCase(recipe)
    with open("C:/Users/king_/WebstormProjects/world-gourmet/content/recipes/" + fileName + ".md", "w+") as file:
        file.write(markdownPage)

def generateMarkdownAndWriteForRecipes(recipes):
    for recipe in recipes:
        markdownPage = getMarkdownPageForRecipe(recipe)
        writeMarkdownPageToFile(recipe, markdownPage)
...
generateMarkdownAndWriteForRecipes(recipes)